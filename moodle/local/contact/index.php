    ...
    foreach ($lines as $linenumbe => $line) {
        ...
        if (count($thisrecipient) === 3) {
            ...
            if ($thisrecipient[0] == $recipient && !empty($thisrecipient[1]) && !empty($thisrecipient[2])) {
                $emails = explode(",",$thisrecipient[1]);
...
// Send the message. Atualizado para v4.5 da linha 148 a 178.
$send = false;
$emails = []; // Inicializa array de emails

// Processa a lista de emails se existir múltiplos destinatários
if (!empty($thisrecipient[1])) {
    $emails = array_map('trim', explode(',', $thisrecipient[1]));
} else {
    // Se não houver destinatário específico, usa o email de suporte
    $emails[] = $email;
}
...
if (!empty($emails)) {
    foreach ($emails as $emailTo) {
        if ($contact->sendmessage($emailTo, $name)) {
            $send = true;
        }
    }
}
...
if ($send) {
    // Share a gratitude and Say Thank You!
